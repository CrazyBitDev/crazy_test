...
class DDQN():
	...
	def get_action(self, state):
		...
		if np.random.random() < self.eps_greedy:
			action = np.random.choice(self.action_space.n - 1) + 1  # avoid selecting emergency action
		else:
	...
	def loop(self, args):		
		...
		for episode in range(args.n_episode):
			...
			while True:
				...
				is_emergency = self.env.is_emergency(is_emergency)
				if is_emergency:
					emergency_mode = self.env.emergency_mode()
					if emergency_mode == 'base':
					...
					action, action_prob = self.get_action(state)
					emergency_mode = None
				new_state, reward, done, info = self.env.step(action, emergency_mode=emergency_mode)
			...
			reason = ""
			if info['goal_reached']: reason = "Goal reached"
			elif info['collision']: reason = "Collision"
			elif info['out_of_bounds']: reason = "Out of bounds"
			elif info['time_out']: reason = "Time out"
			elif info['not_moving']: reason = "Not moving"
			...
			print( f"reward: {logger_dict['reward'][-1]:8.2f} (last_100: {np.mean(reward_last_100):8.2f})", end=" " )
			print( f"step: {logger_dict['step'][-1]:3d} (step_last_100 {int(np.mean(step_last_100)):3d})", end=" " )
			...
			if args.wandb_log:
				wandb.log({
					...
					'Total_Steps': sum(logger_dict['step']),
					'Avg_Step': int(np.mean(step_last_100)),
					#'Avg_Cost': int(np.mean(cost_last_100)*100),
					'Avg_Success': int(np.mean(success_last_100)*100),
